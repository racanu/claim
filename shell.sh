...
[[ $? -ne 0 ]] && docker build --target system-base --build-arg WORKDIR=`pwd` -t $SHELL_IMAGE -f Dockerfile .
...
docker run -it --rm $VOLUMES --env PATH --network host $SHELL_IMAGE /bin/sh
